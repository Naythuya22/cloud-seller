...
def show_ledger_display():
    ...
    st.caption(
        ...
        "**✅ ငွေရှင်းမည်** နှိပ်လိုက်ပါက ဖောက်သည်အတွက် ကျန်ငွေကို ငွေရှင်းပြီးအဖြစ် သိမ်းပါသည်။"
    )
        ...
        for name, group in display_df.groupby("Name", sort=False):
            ...
            if not is_collapsed:
                ...
                if c1.button(f"✅ {name} ငွေရှင်းမည်", key=f"c_{gk}"):
                    ...
                    ok = clear_customer_bill(str(name), settled_by=who)
                    if ok:
                        st.session_state["_ledger_settle_flash"] = f"✅ {name} ငွေရှင်းပြီးပါပြီ။"
