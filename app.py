...
def _rawbt_uri_from_png_bytes(png_bytes: bytes | None, max_uri_chars: int = 800_000) -> str | None:
    """RawBT image mode — PNG ကို rawbt:base64,... (URI အရှည်ကန့်သတ်)"""
    if not png_bytes:
        return None
        ...
        with st.expander(
            ...
            for name, g in day_df.groupby("Name", sort=False):
                ...
                with st.expander("🖼️ RawBT (ပုံ — မြန်မာစာ မှန်ထွက်)", expanded=False):
                    st.caption("စာသား mode မှ မြန်မာစာ ပျက်နိုင်သဖြင့် ပုံကို RawBT သို့ ပို့ရန်။")
            ...
            if _print_mode == "add_print":
                ...
                if _png:
                    ...
                    if _rawbt_img:
                        ...
                        st.warning(
                            "ပြေစာပုံက ကြီးလွန်းသဖြင့် RawBT တိုက်ရိုက်လင့် မဖန်တီးနိုင်ပါ — အောက်မှ PNG ဒေါင်းပြီး မျှဝေပါ။"
                        )
                    st.download_button(
                    ...
                    st.warning(
                        "PNG မဖန်တီးနိုင်ပါ — ဒေသတွင်း Chrome/Edge သို့ Streamlit Cloud တွင် `packages.txt` ဖြင့် chromium ထည့်စစ်ပါ။"
                    )
                st.download_button(
