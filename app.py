import os, json, re, math, shutil, warnings
import pandas as pd
    ...
    for day in days:
        ...
        with st.expander(f"📅 {day} · မှတ်တမ်း {n_lines} ကြောင်း · ဖောက်သည် {n_cust} ယောက် · {day_total:,.0f} Ks", key=f"rbx_d_{dk}"):
            for name, g in day_df.groupby("Name", sort=False):
                ...
                disp.columns = ["မှာယူမှု", "ပမာဏ (Ks)", "ဖျက်ချိန်"]
        ...
        with st.expander(
            ...
            for name, g in day_df.groupby("Name", sort=False):
                ...
                if "SettledBy" in g.columns:
                    ...
                    disp["ငွေရှင်းချသူ"] = "—"
                disp.columns = ["မှာယူမှု", "ပမာဏ (Ks)", "မှာယူသည့်နေ့", "ငွေရှင်းချသူ"]
                st.dataframe(disp, use_container_width=True, hide_index=True)
                st.divider()
...
def show_dashboard():
    ...
    if not df_paid.empty:
        ...
        st.subheader("👥 ဖောက်သည်အလိုက် မရှင်းရသေးငွေ")
        if not df_unpaid.empty:
            unpaid_by_customer = df_unpaid.groupby('Name')['Amount'].sum().sort_values(ascending=False).head(10)
            unpaid_df = pd.DataFrame({
                'ဖောက်သည်အမည်': unpaid_by_customer.index,
                'ကျန်ငွေ': unpaid_by_customer.values
            })
            unpaid_df['ကျန်ငွေ'] = unpaid_df['ကျန်ငွေ'].apply(lambda x: f"{x:,.0f} Ks")
            st.table(unpaid_df)
    else:
...
def show_manual_entry():
    ...
    st.caption("တူညီသော အစားအစာနှင့် ဈေးနှုန်းဖြစ်စေ **တစ်ခါသွင်းတိုင်း စာရင်းတွင် ကြောင်းတစ်ကြောင်းစီ** ခွဲထည့်ပါသည်။")
...
def show_ledger_display():
    ...
    st.caption("စာရင်းမှားရေးမိလျှင် အောက်ပါ ✏️ ပြင်မီနူးဖြင့် နေ့စွဲ / မှာယူမှု / ပမာဏ ပြင်ဆင်နိုင်သည်။ ဒေတာကို ဖျက်မထားဘဲ အစားထိုးသိမ်းပါသည်။")
    ...
    if not display_df.empty:
        for name, group in display_df.groupby("Name", sort=False):
            ...
            is_collapsed = st.session_state.collapsed.get(name, False)
            ...
            if not is_collapsed:
                ...
                html += '<thead> <th>နေ့စွဲ</th><th>မှာယူမှု</th><th>ပမာဏ</th> </thead><tbody>'

                for _, row in group.iterrows():
                    cf = 'class="carry-forward"' if row['Date'] < today else ''
                    html += f'<tr>'
                    html += f'<td {cf}>{row["Date"]}</td>'
                    html += f'<td>{row["Description"]}</td>'
                    html += f'<td style="text-align:right">{row["Amount"]:,.0f} Ks</td>'
                    html += f'</tr>'

                html += f'<tr class="sub-total"><td colspan="2" style="text-align:right">စုစုပေါင်း:</td><td style="text-align:right">{sub:,.0f} Ks</td></tr>'
                html += '</tbody></table>'
                st.markdown(html, unsafe_allow_html=True)

                for idx, row in group.iterrows():
                    short = str(row["Description"])[:28] + ("…" if len(str(row["Description"])) > 28 else "")
                    with st.expander(f"✏️ ပြင်မည် — {short}", key=f"lex_{gk}_{idx}"):
                        rd = _ledger_cell_to_date(row["Date"])
                        ed_date = st.date_input("နေ့စွဲ", value=rd, key=f"ld_{gk}_{idx}")
                        ed_desc = st.text_input("မှာယူမှု (ဥပမာ ကော်ဖီ (၃))", value=str(row["Description"]), key=f"ls_{gk}_{idx}")
                        ed_amt = st.number_input("ပမာဏ (Ks)", min_value=0.0, value=float(row["Amount"]), step=100.0, key=f"la_{gk}_{idx}")
                        if st.button("💾 ပြင်ဆင်ချက်သိမ်းမည်", key=f"lsv_{gk}_{idx}"):
                            ok, err = update_ledger_row(idx, ed_date.strftime("%Y-%m-%d"), ed_desc, ed_amt)
                            if ok:
                                add_memory("agent", f"စာရင်းပြင်ပြီး: {name} — {ed_desc} = {ed_amt:,.0f} Ks")
                                st.success("သိမ်းဆည်းပြီးပါပြီ။")
                                st.rerun()
                            else:
                                st.error(err)
...
.styled-table th,.styled-table td{padding:8px 10px;border:1px solid #E2E8F0;text-align:left}
...
if st.session_state.user is None:
    ...
    if st.session_state.user['role'] == 'admin':
        tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(
            ["🤖 Agent", "📊 Dashboard", "📝 Manual Entry", "🛒 အဝယ်", "📋 Ledger", "🗑️ Recycle Bin", "⚙️ Admin"]
        )
        ...
        tab1, tab2, tab3, tab4 = st.tabs(["🤖 Agent", "📊 Dashboard", "📝 Manual Entry", "🛒 အဝယ်"])
